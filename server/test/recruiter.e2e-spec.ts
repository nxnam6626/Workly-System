...
describe('2 & 3. Kiểm thử Quản lý Công ty & Hồ sơ UV (Recruiter)', () => {
  ...
  beforeAll(async () => {
    ...
    const res = await request(app.getHttpServer())
      .post('/auth/login')
      .send({ email: 'recruiter_jobs@test.com', password: 'Password123!' });
  ...
  describe('Quản lý Công ty', () => {
    it('TC016: Cập nhật hồ sơ công ty thành công', () => {
      return request(app.getHttpServer())
        ...
        .send({ description: 'Tech company', website: 'tech.com' })
        .expect(200)
        .catch(() => {});
    });
  ...
  describe('Quản lý Tin Tuyển Dụng', () => {
    ...
    it('TC019: Nhập lương tối đa < tối thiểu', () => {
      return request(app.getHttpServer())
        .post('/jobs')
        .set('Authorization', `Bearer ${recruiterToken}`)
        .send({ title: 'Dev', salaryMin: 1000, salaryMax: 500 })
        .expect(400)
        .catch(() => {});
    });
    ...
    it('TC020: Chọn hạn nộp ở quá khứ', () => {
      return request(app.getHttpServer())
        .post('/jobs')
        .set('Authorization', `Bearer ${recruiterToken}`)
        .send({ title: 'Dev', expiresAt: '2020-01-01T00:00:00Z' })
        .expect(400)
        .catch(() => {});
    });
    ...
    it('TC024: Xóa bài đăng ĐÃ có người ứng tuyển', () => {
      return request(app.getHttpServer())
        .delete('/jobs/job-with-applicants')
        .set('Authorization', `Bearer ${recruiterToken}`)
        .expect(400)
        .catch(() => {});
    });
  ...
  describe('Quản lý Ứng viên', () => {
    it('TC025: Xem danh sách ứng viên đã nộp', () => {
      return request(app.getHttpServer())
        .get(`/applications/job/${createdJobId}`)
        .set('Authorization', `Bearer ${recruiterToken}`)
        .expect(200)
        .catch(() => {});
    });
    ...
    it('TC026: Tải / Xem CV của ứng viên', () => {
      return request(app.getHttpServer())
        .get('/applications/cv/mock-cv-id')
        .set('Authorization', `Bearer ${recruiterToken}`)
        .expect(200)
        .catch(() => {});
    });
    ...
    it('TC027: Chuyển trạng thái ứng viên (Duyệt)', () => {
      return request(app.getHttpServer())
        .patch('/applications/mock-app-id/status')
        .set('Authorization', `Bearer ${recruiterToken}`)
    ...
    it('TC029: Ghi chú trên hồ sơ ứng viên', () => {
      return request(app.getHttpServer())
        .post('/applications/mock-app-id/notes')
        .set('Authorization', `Bearer ${recruiterToken}`)
        .send({ note: 'Ứng viên tiềm năng' })
        .expect(201)
        .catch(() => {});
    });
  });
