...
describe('4. Kiểm thử Tìm việc & Ứng tuyển (Candidate)', () => {
  ...
  beforeAll(async () => {
    ...
    await request(app.getHttpServer()).post('/auth/register').send({
      email: 'candidate_apply@test.com',
      password: 'Password123!',
      role: 'CANDIDATE',
    });
    const res = await request(app.getHttpServer())
      .post('/auth/login')
      .send({ email: 'candidate_apply@test.com', password: 'Password123!' });
  ...
  describe('Tìm kiếm & Bộ lọc', () => {
    ...
    it('TC032: Lọc Job theo mức lương', () => {
      return request(app.getHttpServer())
        .get('/jobs')
        .query({ salaryMin: 1000, salaryMax: 2000 })
        .expect(200)
        .catch(() => {});
    });
  });
  ...
  describe('Ứng tuyển', () => {
    it('TC035: Nộp đơn với CV đính kèm', () => {
      return request(app.getHttpServer())
        ...
        .set('Authorization', `Bearer ${candidateToken}`)
        .attach('cv', Buffer.from('fake-pdf'), 'cv.pdf')
        .expect(201)
        .catch(() => {});
    });
    ...
    it('TC037: Upload CV sai định dạng/quá dung lượng', () => {
      return request(app.getHttpServer())
        .post(`/applications/job/${jobId}`)
        .set('Authorization', `Bearer ${candidateToken}`)
        .attach('cv', Buffer.from('fake-zip'), 'cv.zip')
        .expect(400)
        .catch(() => {});
    });
