...
import { EVASION_REGEX } from '@/modules/intelligence/ai/ai-moderation.service';
...
export class JobModerationService {
  ...
  async checkAndAutoLockRecruiter(recruiterId: string) {
    ...
    if (newViolations >= this.VIOLATION_LIMIT) {
      ...
      this.logger.warn(
        `Recruiter ${recruiterId} auto-locked due to ${newViolations} violations.`,
      );
    }
  }
  ...
  async preCheck(dto: CreateJobPostingDto) {
    ...
    if (containsBadWords) {
      return {
        ...
        flags: foundWords,
        feedback: [
          'Vui lòng loại bỏ các từ ngữ không phù hợp hoặc thông tin liên hệ cá nhân (số điện thoại, link mạng xã hội) khỏi mô tả.',
        ],
        usedAI: false,
      };
    }
    ...
    return {
      ...
      suggestedAction:
        modResult.score < 70
          ? 'Sửa lại JD để được duyệt tự động, hoặc gửi Admin duyệt thủ công.'
          : 'JD đạt chuẩn, có thể đăng ngay.',
    };
  }
}
