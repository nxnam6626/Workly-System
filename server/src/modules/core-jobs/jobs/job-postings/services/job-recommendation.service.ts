...
export class JobRecommendationService {
      ...
      matches.map(async (m) => {
        ...
        return {
          ...
          analysis: {
            ...
            experienceMatch:
              (candidate?.cvs?.[0]?.parsedData as any)?.totalYearsExp >=
              (jobReqs?.minExperienceYears || 0),
            totalYearsExp:
              (candidate?.cvs?.[0]?.parsedData as any)?.totalYearsExp || 0,
  ...
  async getRecommendations(userId: string) {
    ...
    return matches.filter((m) => m.score >= 60);
  }
}
