...
export class ApplicationInterviewService {
  ...
  async getAvailableSlots(applicationId: string) {
    ...
    if (!application || (application.appStatus !== 'INTERVIEWING' && application.appStatus !== 'RESCHEDULE_REQUESTED')) {
      return [];
    }
  ...
  async candidateScheduleInterview(applicationId: string, candidateUserId: string, date: string, time: string) {
    const application = await this.prisma.application.findUnique({
      where: { applicationId },
      include: { 
    ...
    const settings: any = (application.jobPosting.recruiter as any)?.interviewSettings || {};
    const interviewLocation = settings.defaultLocation || application.jobPosting.company?.address || 'Trao đổi qua tin nhắn';
    ...
    if (application.jobPosting.recruiter?.userId) {
      try {
        ...
        this.messagesGateway.server.to(`user_${application.jobPosting.recruiter.userId}`).emit('dashboard.sync');
        this.messagesGateway.server.to(`user_${application.candidate.userId}`).emit('notification');
        
        // Emit newMessage to both parties
        this.messagesGateway.server.to(`user_${application.candidate.userId}`).emit('newMessage', savedMessage);
        this.messagesGateway.server.to(`user_${application.jobPosting.recruiter.userId}`).emit('newMessage', savedMessage);
      } catch (err) {
  ...
  async checkUpcomingInterviews() {
    try {
      ...
      for (const app of upcomingApps) {
        if (!app.interviewDate || !app.interviewTime || !app.candidate?.userId) continue;
