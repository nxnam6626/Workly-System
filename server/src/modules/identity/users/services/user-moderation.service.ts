...
export class UserModerationService {
  ...
  async unlockWithProbation(userId: string) {
    ...
    return {
      message:
        'Đã mở khóa tài khoản và đưa vào danh sách Thử thách (Reset vi phạm).',
    };
  }
  ...
  async banUser(userId: string) {
    ...
    if (user.email === 'admin@workly.com')
      throw new BadRequestException(
        'Không thể ban tài khoản Quản trị viên tối cao.',
      );
    await this.prisma.user.update({
      where: { userId },
      data: { status: 'BANNED' },
  ...
  async updateAdminPermissions(
    ...
    const isSupreme =
      permissions.includes('ALL') || permissions.includes('SUPER_ADMIN');
