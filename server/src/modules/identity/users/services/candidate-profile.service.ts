...
export class CandidateProfileService {
  ...
  async updateCandidateProfile(userId: string, dto: UpdateCandidateProfileDto) {
    ...
    this.logger.log(
      `Updating candidate profile for candidateId: ${candidateId}`,
    );
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      await tx.candidate.update({
        ...
        data: {
          ...
          ...(dto.totalYearsExp !== undefined && {
            totalYearsExp: dto.totalYearsExp,
          }),
    ...
    try {
      ...
      this.logger.error(
        `Failed to trigger matching for userId: ${userId}`,
        err.stack,
      );
    }
  }
