    ...
    if (recruiterUserId) {
      ...
      if (recruiter) {
        ...
        const jobTitleMap = new Map(
          activeJobs.map((j) => [j.jobPostingId, j.title]),
        );
        ...
        enrichedData = data.map((candidate) => {
          ...
          const candidateMatches =
            matchesByCandidate.get(candidate.candidateId) || [];
          candidateMatches.sort((a, b) => b.score - a.score);

          const maxScore =
            candidateMatches.length > 0 ? candidateMatches[0].score : 0;
          const bestJobId =
            candidateMatches.length > 0
              ? candidateMatches[0].jobPostingId
              : null;
          const bestJob = bestJobId ? jobTitleMap.get(bestJobId) : '';

          const matchDetails = candidateMatches.slice(0, 3).map((m) => ({
