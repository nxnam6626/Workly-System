...
export class MessageAttachmentService {
  ...
  async sendAttachment(
    ...
    if (mimeType.startsWith('image/')) {
      const moderation = await this.aiService.moderateChatImageBuffer(
        fileBuffer,
        mimeType,
      );
      ...
      extractedText = await this.aiService.extractTextFromBuffer(
        fileBuffer,
        mimeType,
      );
      const normalizedText = extractedText.replace(/[\s\.\-\_]/g, '');
      isEvasion =
        EVASION_REGEX.test(extractedText) ||
        EVASION_REGEX.test(normalizedText) ||
        PROFANITY_REGEX.test(extractedText);
    }
      ...
      if (user) {
        ...
        if (newViolations >= 3) {
          ...
          exceptionToThrow = {
            ...
            message:
              'Tài khoản của bạn đã bị khóa vĩnh viễn vì chia sẻ tệp/liên kết vi phạm 3 lần.',
            email: user.email,
          };
        } else {
          await this.prisma.user.update({
