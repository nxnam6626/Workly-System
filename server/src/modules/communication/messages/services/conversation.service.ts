...
import {
  EVASION_REGEX,
  PROFANITY_REGEX,
} from '../constants/message-violation.constants';
...
export class ConversationService {
  ...
  async getMessages(
    ...
    return messages.reverse().map((msg) => {
      if (
        msg.content.startsWith('[HIDDEN_VIOLATION]') &&
        msg.senderId !== userId
      ) {
        return {
          ...msg,
  ...
  async sendMessage(
      ...
      if (user) {
        ...
        if (newViolations >= 3) {
          ...
          exceptionToThrow = {
            ...
            message:
              'Tài khoản của bạn đã bị khóa vĩnh viễn do vi phạm quy tắc cộng đồng 3 lần.',
            email: user.email,
          };
        } else {
          await this.prisma.user.update({
    ...
    await this.prisma.conversation.update({
      where: { conversationId },
      data: {
        lastMessage: finalContent.startsWith('[HIDDEN_VIOLATION]')
          ? 'Tin nhắn đã bị ẩn do vi phạm quy tắc hệ thống.'
          : finalContent,
        updatedAt: new Date(),
        isRead: false,
      },
    ...
    const senderMessage = {
      ...message,
      content,
      isViolated: finalContent.startsWith('[HIDDEN_VIOLATION]'),
    };
    if (exceptionToThrow)
      throw new BadRequestException({
        ...exceptionToThrow,
        savedMessage: senderMessage,
      });
  ...
  async getUserViolations(userId: string) {
    ...
    return messages.map((m) => ({
      ...
      sentAt: m.sentAt,
      conversationName:
        m.conversation?.candidate?.fullName ||
        m.conversation?.recruiter?.company?.companyName ||
        'Không rõ',
    }));
  }
