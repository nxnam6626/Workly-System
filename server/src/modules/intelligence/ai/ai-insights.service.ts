...
export class AiInsightsService {
  ...
  async generateRecruiterInsights(
    ...
    const isCacheFresh =
      ...
      Date.now() - new Date(recruiter.aiInsightsCachedAt).getTime() <
        CACHE_TTL_MS;
    ...
    if (
      !forceRefresh &&
      cachedPayload &&
      (recruiter.aiInsightsCacheKey === cacheKey || isCacheFresh)
    ) {
      this.logger.log(
        `[AiInsightsService] Returning cached insights for recruiter ${recruiter.recruiterId}`,
      );
    ...
    return this.aggregateService.generateAggregateInsights(
      recruiter,
      stats,
      finalJdScores,
      cacheKey,
