...
export class AiChatResponseService {
  ...
  async generateResponse(message: string): Promise<string> {
    ...
    if (process.env.GROQ_API_KEY) {
      try {
            ...
            messages: [
              {
                role: 'system',
                content:
                  'Bạn là chuyên gia tư vấn Tuyển Dụng và Nhân Sự (Workly-AI). NGUYÊN TẮC TỐI THƯỢNG: TUYỆT ĐỐI KHÔNG trả lời các chủ đề ngoài lề. Nếu người dùng hỏi ngoài lề, chỉ trả lời duy nhất: "Tôi là Trợ lý AI Tuyển Dụng, tôi chỉ có thể giúp bạn các vấn đề liên quan đến tuyển dụng, tối ưu JD và nhân sự."',
              },
        ...
        this.logger.warn(
          `[AiChatResponseService] Groq generateResponse failed: ${e.message}. Falling back to Gemini...`,
        );
      }
    }
      ...
      for (const modelName of modelsToTry) {
        try {
          const model = this.genAI.getGenerativeModel({
            model: modelName,
            systemInstruction:
              'Bạn là chuyên gia tư vấn Tuyển Dụng và Nhân Sự. NGUYÊN TẮC: TUYỆT ĐỐI KHÔNG trả lời chủ đề ngoài lề. Nếu hỏi ngoài lề, từ chối và nói bạn chỉ hỗ trợ tuyển dụng.',
          });
          ...
          this.logger.warn(
            `[AiChatResponseService] generateResponse error with ${modelName}.`,
          );
          await SLEEP(500);
        }
      }
  ...
  async expandSearchQuery(message: string): Promise<string[]> {
    ...
    if (process.env.GROQ_API_KEY) {
      try {
        ...
        this.logger.warn(
          `[AiChatResponseService] Groq expandSearchQuery failed: ${e.message}. Falling back to Gemini...`,
        );
      }
    }
  ...
  async extractJobData(
    message: string,
  ): Promise<{ jobData: any; usedAI: string } | null> {
            ...
            messages: [
              {
                role: 'system',
                content:
                  'You are an elite HR Recruiter API. You MUST output extremely long, highly detailed, and professional job descriptions in Vietnamese.',
              },
  ...
  async *generateStreamResponse(
    ...
    if (!success) {
      this.logger.error(
        `Streaming failed, trying direct fallback. Error: ${lastError?.message}`,
      );
      const fallbacks = ['gemini-1.5-flash-8b'];
