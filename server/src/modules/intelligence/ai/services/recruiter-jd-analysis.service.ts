...
export class RecruiterJdAnalysisService {
  ...
  async analyzeMissingJds(activeJobs: any[], finalJdScores: any[]) {
    ...
    for (const job of activeJobs) {
      const struct = job.structuredRequirements || {};
      const aiAnalysis = struct.aiAnalysis;

      if (
        aiAnalysis &&
        aiAnalysis.version === 15 &&
  ...
  private calculateRuleBasedScore(job: any) {
    ...
    if (!hasSalary) {
      ...
      detailedAdvice.push(
        'Cần bổ sung mức lương tối thiểu và tối đa để tăng ngay lập tức lòng tin từ ứng viên.',
      );
    } else {
    ...
    if (descLength < 300) {
      ...
      detailedAdvice.push(
        'Mô tả công việc cực kỳ sơ sài khiến ứng viên không hình dung được họ sẽ phải làm gì.',
      );
    ...
    return {
      ...
      strengths,
      detailedAdvice,
      autoFixedByAI: job.structuredRequirements?.autoFixedByAI,
    };
  }
  ...
  private async runLlmAnalysis(jobs: any[], finalJdScores: any[]) {
    const summary = jobs.map((j) => ({
      ...
      descriptionContent: (j.description || '')
        .replace(/<[^>]*>?/gm, '')
        .substring(0, 250),
      reqContent: (j.requirements || '')
        .replace(/<[^>]*>?/gm, '')
        .substring(0, 200),
        ...
        const groqRes = await axios.post(
          'https://api.groq.com/openai/v1/chat/completions',
          {
            ...
            messages: [
              {
                role: 'system',
                content: 'You are an HR AI Assistant. Always return JSON.',
              },
