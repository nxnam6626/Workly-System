    ...
    try {
      ...
      for (const modelName of modelsToTry) {
        try {
          ...
          this.logger.warn(
            `[AiExtractionService] Local PDF extraction failed with ${modelName}.`,
          );
          await SLEEP(500);
        }
      }
    ...
    try {
      ...
      for (const modelName of modelsToTry) {
        try {
          ...
          this.logger.warn(
            `[AiExtractionService] Buffer extraction failed with ${modelName}.`,
          );
          await SLEEP(500);
        }
      }
    ...
    try {
      ...
      for (const modelName of modelsToTry) {
        try {
          ...
          this.logger.warn(
            `[AiExtractionService] URL PDF extraction failed with ${modelName}.`,
          );
          await SLEEP(500);
        }
      }
