...
Fix the SQL query and return ONLY the corrected SQL string.`;
    ...
    if (process.env.GROQ_API_KEY) {
      try {
            ...
            messages: [
              {
                role: 'system',
                content:
                  'You are an expert SQL architect. Output ONLY the raw SQL query, no markdown blocks.',
              },
        ...
        const sql = groqRes.data.choices[0].message.content;
        return sql
          .replace(/```sql/gi, '')
          .replace(/```/gi, '')
          .trim();
        ...
        console.warn(
          `[SqlGenerator] Groq failed: ${e.message}. Falling back...`,
        );
      }
    }
