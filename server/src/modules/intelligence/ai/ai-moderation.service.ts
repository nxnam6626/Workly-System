...
export class AiModerationService {
  ...
  async moderateChatImageBuffer(
    buffer: Buffer,
    mimeType: string,
  ): Promise<{ isEvasion: boolean; textExtracted: string }> {
    ...
    try {
      ...
      const isEvasion =
        EVASION_REGEX.test(extracted) || EVASION_REGEX.test(normalized);
  ...
  async moderateJobContent(
    ...
    if (jobTier === 'URGENT' || jobTier === 'PROFESSIONAL') {
      depthInstruction =
        'LƯU Ý ĐẶC BIÊT: Đây là tin tuyển dụng VIP (Nổi bật/Tuyển gấp). Bắt buộc nhận xét ĐẦY ĐỦ CHI TIẾT rành mạch, phân tích sâu logic kỹ năng/quyền lợi và nêu rõ ví dụ để HR cải thiện chất lượng JD.';
    } else {
      depthInstruction =
        'LƯU Ý: Đây là tin tuyển dụng Thường. Chỉ cần nhận xét ngắn gọn, súc tích chỉ ra những điểm sai sót lớn nhất.';
    }
...
{"score":85,"safe":true,"flags":["thiếu mô tả chi tiết quyền lợi"],"reason":"Tóm tắt nhanh lý do chấm điểm","feedback":["Lời khuyên 1","Lời khuyên 2"]} (Lưu ý mảng feedback bắt buộc phải là Array gồm các chuỗi String)`;
    ...
    if (process.env.GROQ_API_KEY) {
      try {
        const groqRes = await axios.post(
          'https://api.groq.com/openai/v1/chat/completions',
          {
            ...
            messages: [
              {
                role: 'system',
                content: 'You are a strict JD Moderator. Always return JSON.',
              },
        ...
        this.logger.log(
          `[AiModerationService] moderateJobContent: score=${parsed.score}, safe=${parsed.safe} (Groq)`,
        );
        return {
        ...
        this.logger.warn(
          `[AiModerationService] Groq moderateJobContent failed: ${e.message}. Falling back to Gemini...`,
        );
      }
    }
