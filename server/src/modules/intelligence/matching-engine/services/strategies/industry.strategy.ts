    ...
    try {
      ...
      if (jobCategories.length === 0)
        return {
          ...
          details: {
            jobCategories: [],
            cvIndustry: cvIndustry || 'Không rõ',
            isMatch: true,
            message: 'Không yêu cầu ngành nghề',
          },
        };
      ...
      const isMatch = jobCategories.some((cat) => {
        ...
        const matchesCvIndustry = cvIndustry
          ? catLower.includes(cvIndustry) || cvIndustry.includes(catLower)
          : false;
        const matchesCandidateIndustries = candidateIndustries.some(
          (ci: string) =>
            ci && (ci.includes(catLower) || catLower.includes(ci)),
        );

        return matchesCvIndustry || matchesCandidateIndustries;
      });

      const combinedCvIndustry = [
        cv.parsedData?.industry,
        ...(cv.candidate?.industries || []),
      ]
        .filter(Boolean)
        .join(', ');
      ...
      return {
        ...
        details: {
          jobCategories,
          cvIndustry: combinedCvIndustry || 'Chưa cập nhật',
          isMatch,
        },
      };
    } catch (error) {
