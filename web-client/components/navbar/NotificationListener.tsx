  ...
  useEffect(() => {
    ...
    const handleViolationWarn = (msg: string) => {
      toast.custom(
        (t) => (
          <div
            className={`${t.visible ? 'animate-in slide-in-from-top-2 fade-in duration-300' : 'animate-out slide-out-to-top-2 fade-out duration-300'
    ...
    const handleAccountLocked = (data?: any) => {
      const displayMsg = data?.message || (typeof data === 'string' ? data : 'Tài khoản của bạn đã bị khoá bởi Quản trị viên do vi phạm điều khoản.');
      toast.custom(
        (t) => (
          <div
            className={`${t.visible ? 'animate-in zoom-in-95 fade-in duration-300' : 'animate-out zoom-out-95 fade-out duration-300'
